...
print('Temporal duration tables to process:')
print(temporal_duration_tables)
...
for dur, file_path in temporal_duration_tables.items():
    ...
    for i,table in enumerate(table_start_indexes):
        ...
        print(f'Processing {dur} {table_title}')
...
# %%
# Create a plot of just Quartile 1 distributions for 24h duration
# that means columns: Atlas 14 Storm24hDistribution Q1 10,etc.
df_quartile1_24h = df_all[["time", "Atlas 14 Storm24hDistribution Q1 90", "Atlas 14 Storm24hDistribution Q1 80", "Atlas 14 Storm24hDistribution Q1 70", "Atlas 14 Storm24hDistribution Q1 60", "Atlas 14 Storm24hDistribution Q1 50", "Atlas 14 Storm24hDistribution Q1 40", "Atlas 14 Storm24hDistribution Q1 30", "Atlas 14 Storm24hDistribution Q1 20", "Atlas 14 Storm24hDistribution Q1 10"]]
# for each column rename to match the patterns "24hr Q1 90%", etc. (drop "Atlas 14" and add % to the end of the number)
df_quartile1_24h.columns = ["time"] + [f"24hr Q1 {col.split()[-1]}%" for col in df_quartile1_24h.columns[1:]]
fig_quartile1_24h = px.line(df_quartile1_24h, x="time", y=df_quartile1_24h.columns[1:], title="Atlas 14 Temporal Distributions for 24h Duration - 1st Quartile")

fig_quartile1_24h.update_layout(
    xaxis_title="Time (hours)",
...
# make the 90% and the 10% lines thicker and dashed
fig_quartile1_24h.update_traces(selector=dict(name="24hr Q1 90%"), line=dict(width=6, dash='dashdot'))
fig_quartile1_24h.update_traces(selector=dict(name="24hr Q1 10%"), line=dict(width=6, dash='dash'))





# since the labels of each trace 90%, 80%, etc, show up on the plot in order, lets color them in a gradient using turbo colorscale
colors = px.colors.sequential.Turbo
num_lines = len(df_quartile1_24h.columns) - 1
for i, col in enumerate(df_quartile1_24h.columns[1:]):
    color_index = int(i / (num_lines - 1) * (len(colors) - 1))
    fig_quartile1_24h.update_traces(selector=dict(name=col), line=dict(color=colors[color_index], width=4))

# reverse the legend order so 10% is on top
fig_quartile1_24h.update_layout(legend=dict(traceorder="reversed"))

# update the x-axis labels to show for each value
fig_quartile1_24h.update_xaxes(tickmode='linear', tick0=0, dtick=1)

fig_quartile1_24h.write_html("Atlas14_Q1_24h.html")
# %%
# Create a plot for the 10% Occurence across all quartiles for 24h duration
# columns will look like: Atlas 14 Storm24hDistribution Q1 10, Atlas 14 Storm24hDistribution Q2 10, etc.
df_10_percent_24h = df_all[["time", "Atlas 14 Storm24hDistribution Q1 10", "Atlas 14 Storm24hDistribution Q2 10", "Atlas 14 Storm24hDistribution Q3 10", "Atlas 14 Storm24hDistribution Q4 10", "Atlas 14 Storm24hDistribution ALL 10"]]
# rename the columns to match the pattern "24hr Q1 10%", etc.
df_10_percent_24h.columns = ["time"] + [f"24hr {col.split()[-2]} {col.split()[-1]}%" for col in df_10_percent_24h.columns[1:]]
fig_10_percent_24h = px.line(df_10_percent_24h, x="time", y=df_10_percent_24h.columns[1:], title="Atlas 14 Temporal Distributions for 24h Duration - 10% Occurrence Across All Quartiles")
fig_10_percent_24h.update_layout(
    xaxis_title="Time (hours)",
    yaxis_title="Precipitation (%)",
    legend_title="Distribution"
)
...
# make the Q1 and Q4 lines thicker and dashed
# fig_10_percent_24h.update_traces(selector=dict(name="24hr Q1 10%"), line=dict(width=6, dash='dashdot'))
# fig_10_percent_24h.update_traces(selector=dict(name="24hr Q4 10%"), line=dict(width=6, dash='dash'))

# color the lines using the turbo colorscale with a custom order of: Q1, ALL, Q2, Q3, Q4
custom_order = ["24hr Q1 10%", "24hr ALL 10%", "24hr Q2 10%", "24hr Q3 10%", "24hr Q4 10%"]
colors = px.colors.sequential.Turbo
# reverse the colors so Q1 is darkest and Q4 is lightest
colors = colors[::-1]
num_lines = len(custom_order)
for i, col in enumerate(custom_order):
    color_index = int(i / (num_lines - 1) * (len(colors) - 1))
    fig_10_percent_24h.update_traces(selector=dict(name=col), line=dict(color=colors[color_index], width=4))
